...
from urllib import parse 	#导入解析模块
from aiohttp import web
from apis import APIError 	#导入自定义的api错误模块

#定义装饰器，将一个函数映射为一个URL处理函数
def get(path):
	def decorator(func):
		#装饰器作用是解决一些函数签名的问题
		#比如若没有改装饰器，wrapper.__name__将为“wrapper”
		#加了装饰器之后，wrapper.__name__将为func.__name__
		...
		if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
			...
			if request.method =='POST':
				...
				if ct.startswith('application/json'):
					...
					# post, content type字段指定的消息主体是json字符串,且解码得到参数为字典类型的,将其赋给变量kw
					kw =params 	
